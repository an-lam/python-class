"""
 Purpose: Read an option file and display the top 10 biggest open interest
 options.
 Rev. Dec. 2022: chop off the year on the GUIs
"""
...
"""
Plot charts of top CALL and top PUT open interests
"""
def plot_charts2(put_options, put_oi_ix, call_options, call_oi_ix, strike_ix):
    ...
    for i in range(0, 11):
        ...
        # Get rid of the year
        exp_strike = exp_strike.replace(" 23", "")
        # print("exp_strike = ", exp_strike)
...
def display_by_exp(opt_lst, exp_ix):

    # Show in GUI the next 3 Friday expirations and allow user to choose
    target_exp = "Figure out next Friday expiration"

    # Build the list containing only target_exp

    # Sort the list by strike prices

    # Show the bar chart: open interest and strike prices
...
# Save options in a list of tuple as "options"
options = []
already_print = False
    ...
    for row in csv_reader:
        ...
        if ("Exp" in row) or ("<empty>" in row):
            if not already_print:
                ...
                exp_ix = row.index("Exp")
                print("call_oi_col =", call_oi_col, "\t put_oi_col = ",
                      put_oi_col, "\tstrike_ix = ", strike_ix, "\t exp_ix = ", exp_ix)
        else:
            #print("row = ", row)
            if row and row[0] != '':
                record = tuple(row)
...
# Display max pain OI of the next Friday expiration
# TODO:
display_by_exp(options, exp_ix)
