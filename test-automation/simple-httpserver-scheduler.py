    ...
    def process_all(self, request):
        ...
        for test in test_names:
            ...
            if test.find("test_ls") != -1:
                ...
                #result[test] = out
                #SimpleHTTPRequestHandler.scheduler.add_test("test_ls", datetime.now()+timedelta(seconds=10))
                self.__class__.scheduler.add_test("test_ls", datetime.now() + timedelta(seconds=10))
                result[test] = "test_ls has been added to schedule DB."
            elif test.find("test_cd") != -1:
                SimpleHTTPRequestHandler.scheduler.add_test("test_cd", datetime.now()+timedelta(seconds=15))
                result[test] = "test_cd has been added to schedule DB."
            else:
